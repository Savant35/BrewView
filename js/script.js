...
//grabs user input and checks for suggestions to give
inputBox.onkeyup = function(){
  let result = [];
  let input = inputBox.value;

  // 1. Check if there is text in the box
  if(input.length){
    result = availableKeyWords.filter((keyword) =>{
      return keyword.toLowerCase().includes(input.toLowerCase());
    });

    // 2. ONLY display if we actually found matches
    if (result.length > 0) {
        display(result);
        // SHOW THE BOX
        suggestionBox.style.display = "block";
    } else {
        // HIDE THE BOX (No matches found)
        suggestionBox.style.display = "none";
    }
  } else {
    // 3. HIDE THE BOX (Input is empty)
    suggestionBox.style.display = "none";
  }
}
...
function display(result) {
  const content = result.map((list) => {
    return "<li class='suggestion-item'>" + list + "</li>";
  });
...
suggestionBox.addEventListener("click", function(e) {
  // Check if the user clicked on an LI item
...
window.addEventListener("click", function(event) {
  ...
  // Check if the popup is actually open right now
